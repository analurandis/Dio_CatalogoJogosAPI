...
namespace Api.Catalogo.Controllers.V1
{
    ...
    public class JogosController : ControllerBase
    {
        ...
        public async Task<ActionResult<JogoViewModel>> InserirJogo([FromBody] JogoInputModel jogo)
        {
            try
            {
                var jogoInsert = await _jogoService.Inserir(jogo);
                return Ok(jogoInsert);
            }
            catch (Exception ex)
            {

                return UnprocessableEntity("Já existe um jogo com este nome e para esta produtora");
            }
